function CqNgChart($element, quoteFeed, UIContextFactory){
	...
	ctrl.$postLink=function(){
		...
		ctrl.initChart();
		if(ctrl.parent) {			
			UIContextFactory.create(ctrl.ciq);
			ctrl.parent.cqNgChart=ctrl; //here we give the UI (the parent) access to ourself (the chart)				
		}
	...
	ctrl.initChart=function(){
		...
		ctrl.attachFeed();
		ctrl.ciq.container.style.width=STX.pageWidth()
		ctrl.ciq.container.style.height=STX.pageHeight()-300;
		if(ctrl.symbolInput) {
			ctrl.ciq.newChart(ctrl.symbolInput);
